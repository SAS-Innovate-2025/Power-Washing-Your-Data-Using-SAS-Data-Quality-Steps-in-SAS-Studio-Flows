...
1. Edit the macro parameters as follows:
   ...
   * inTable=**&_input1**
     > &#9998; **&_input1** is a default macro variable which stores the input table name for a node. This option is used because the input table is a temporary table created by the Match Codes step, and temporary tables are given a random name by default.
   * outTable=**&_output1**
...
1. **(REQUIRED)** Select ![Steps Menu Icon](img/StepsIcon.png) to return to the **Steps** pane.
...
1. Add the following columns to the *Select* tab by double-clicking the column name on the left. Update the column attributes as specified below:
